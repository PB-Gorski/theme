...
<div id="<?php echo esc_attr($ymc_filter_layout); ?>-<?php echo esc_attr($c_target); ?>" class="filter-layout <?php echo esc_attr($ymc_filter_layout); ?> <?php echo esc_attr($ymc_filter_layout); ?>-<?php echo esc_attr($layout_id); ?> <?php echo esc_attr($ymc_filter_layout); ?>-<?php echo esc_attr($layout_id); ?>-<?php echo esc_attr($c_target); ?>">
  ...
  ?>
    ...
    <div class="js-heightAnimation filterContainer h-auto transition-all duration-[300ms] ease">
    ...
    <p class="js-foundedPostOnStart custom-posts-found container mx-auto bg-white w-full flex items-center justify-center text-center z-[1] absolute translate-x-[-50%] desktop:text-[24px] text-[18px] text-center" style="left:50%; transition: all .3s ease;width:100%;height:110px" data-aos="fade-up" data-aos-offset="30">Znaleziono <?php echo wp_count_posts('mieszkania')->publish; ?> ofert pasujących do Twoich kryteriów <span class="text-[16px] text-[#8a8f99]">(wszystkich ogłoszeń <?php echo wp_count_posts('mieszkania')->publish; ?>)</span></p>
