...
<div id="<?php echo esc_attr($ymc_filter_layout); ?>-<?php echo esc_attr($c_target); ?>" class="filter-layout <?php echo esc_attr($ymc_filter_layout); ?> <?php echo esc_attr($ymc_filter_layout); ?>-<?php echo esc_attr($layout_id); ?> <?php echo esc_attr($ymc_filter_layout); ?>-<?php echo esc_attr($layout_id); ?>-<?php echo esc_attr($c_target); ?>">
  ...
  ?>
    ...
    <div class=" filter-entry" data-terms="<?php echo esc_attr($all_terms); ?>">
      ...
      foreach ($result_tax as $tax) {
        ...
        if (get_taxonomy($select_term)->label == 'Cena') { ?>
          ...
          <div class="btn-wrapper dropdown-filter bg-white flex items-center justify-center self-start">
            <div class="wp-block-group btn-more dropdown-filter mt-[5px] w-full h-[62px] bg-[#f5f7fa] hover:bg-[#f2f2f2] flex gap-[10px] justify-center items-center cursor-pointer border-b-[3px] border-b-[#dcdee3]">
    ...
    <p class="js-foundedPostOnStart container mx-auto bg-white w-full absolute translate-x-[-50%] z-[2] text-[24px] text-center" style="left:50.5%;top:177px" data-aos="fade-up" data-aos-offset="30">Znaleziono <?php echo wp_count_posts('mieszkania')->publish; ?> ofert pasujących do Twoich kryteriów <span class="text-[16px] text-[#8a8f99]">(wszystkich ogłoszeń <?php echo wp_count_posts('mieszkania')->publish; ?>)</span></p>
