...
<div id="<?php echo esc_attr($ymc_filter_layout); ?>-<?php echo esc_attr($c_target); ?>" class="filter-layout <?php echo esc_attr($ymc_filter_layout); ?> <?php echo esc_attr($ymc_filter_layout); ?>-<?php echo esc_attr($layout_id); ?> <?php echo esc_attr($ymc_filter_layout); ?>-<?php echo esc_attr($layout_id); ?>-<?php echo esc_attr($c_target); ?>">
  ...
  ?>
      ...
      foreach ($result_tax as $tax) {
        ...
        if (get_taxonomy($select_term)->label == 'Cena') { ?>
          ...
          <div class="btn-wrapper bg-white h-[120px] flex items-center justify-center" style="margin-right:2px;">
            <div class="wp-block-group btn-more relative mt-[5px] w-[80px] bg-[#ebeef2] flex gap-[10px] justify-center items-center cursor-pointer border-b-[3px] border-b-[#dcdee3] before:content-[``] before:inline-block before:absolute before:bottom-0 before:left-0 before:w-full before:min-h-[0%] hover:before:min-h-[100%] before:bg-[#e1e1e1] before:transition-height before:duration-300 before:ease cursor-pointer" style="height:50px;width:170px">
              <p class="text-[18px] font-bold">Więcej opcji</p>
            </div>
          </div>
          <div class="btn-search btn-wrapper bg-white h-[120px] flex items-center justify-center">
            <div class="wp-block-group group btn-search mt-[5px] relative bg-primaryRed text-white w-[170px] desktop:w-[140px] h-[50px] flex items-center justify-center gap-[15px] before:content-[``] before:inline-block before:absolute before:bottom-0 before:left-0 before:w-full before:min-h-[0%] hover:before:min-h-[100%] before:bg-[#9C0927] before:transition-height before:duration-300 before:ease cursor-pointer" style="height:50px;width:170px">
    ...
    <p class="js-foundedPostOnStart container mx-auto bg-white w-full absolute translate-x-[-50%] z-[2] text-[24px] text-center" style="left:50.5%;top:160px; transition: all .3s ease-in-out;" data-aos="fade-up" data-aos-offset="30">Znaleziono <?php echo wp_count_posts('mieszkania')->publish; ?> ofert pasujących do Twoich kryteriów <span class="text-[16px] text-[#8a8f99]">(wszystkich ogłoszeń <?php echo wp_count_posts('mieszkania')->publish; ?>)</span></p>
